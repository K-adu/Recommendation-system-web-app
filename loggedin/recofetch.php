...
// $Email=$Email+'reco';
// $query = "
/*SELECT * FROM table2 where CATEGORYID='4'  ORDER BY id ASC limit 0,6*/
// //";
// session_start();
$Email=$_SESSION['Email'];
$reco = 'reco';
$Emailreco=$Email.$reco;
//$Email=$Email+'reco';
$query = "
SELECT * FROM table2 WHERE id IN (SELECT bookid FROM $Emailreco) ORDER BY id ASC limit 5,4
 ";
...
{$name=$row['TITLE'];
 ...
 <a href="readlatersent.php?title='.$name.'&author='.$name2.'" style="color:white;margin-left:120px;font-size:13px;">Add TO MY BOOKLISTS</a></th>
 ';
}
echo $output;

function counts($business_id, $connect)
{
 $output = 0;
