...
$query = "
SELECT * FROM table2 where CATEGORYID='25'  ORDER BY id ASC limit 0,7
";
...
{$name=$row['TITLE'];
  $name2=$row['AUTHOR'];
 $rating = countings_ratings($row['id'], $connect);
 $color = '';
 ...
 <label style="color:rgb(226, 226, 226);padding-left:50px;font-size:13px;">'.$row["CATEGORY"].'</label><br>
 <a href="readlatersent.php?title='.$name.'&author='.$name2.'" style="color:white;margin-left:50px;">Add TO MY BOOKLISTS</a></th>
 ';
}
echo $output;

function countings_ratings($business_id, $connect)
{
 $output = 0;
