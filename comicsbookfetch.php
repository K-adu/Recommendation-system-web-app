...
$query = "
SELECT * FROM table2 where CATEGORYID='5'  ORDER BY id ASC limit 0,1
";
...
{
 $rating = counts_ratings($row['id'], $connect);
 $color = '';
 $output .= '<img src='.$row['IMAGEURL'].' width=100 height=130 style="margin-left:80px;">
 ...
 <label style="color:rgb(226, 226, 226);padding-left:80px;">'.$row["CATEGORY"].'</label><br><br>
 ';
}
echo $output;

function counts_ratings($business_id, $connect)
{
 $output = 0;
