...
$query = "
SELECT * FROM table2 where CATEGORYID='16'  ORDER BY id ASC limit 13,6
";
...
{
 $rating = counted($row['id'], $connect);
 $color = '';
 $output .= '<th><img src='.$row['IMAGEURL'].' width=100 height=130 style="margin-left:80px; margin-top:0px;">
 ...
 <p style="color:rgb(226, 226, 226); margin-left:80px;">'.$row["TITLE"].'</p>
 <p style="color:rgb(226, 226, 226);padding-left:80px;">'.$row["AUTHOR"].'</p>
 <label style="color:rgb(226, 226, 226);padding-left:80px;">'.$row["CATEGORY"].'</label></th>
 ';
}
echo $output;

function counted($business_id, $connect)
{
 $output = 0;
