...
$query = "
SELECT * FROM table2 where CATEGORYID='4'  ORDER BY id ASC limit 0,4
";
...
{$name=$row['TITLE'];
  $name2=$row['AUTHOR'];
 $rating = count_rat($row['id'], $connect);
 $color = '';
 ...
 <label style="color:rgb(226, 226, 226);padding-left:120px;font-size:13px;">'.$row["CATEGORY"].'</label></th>
 ';
}
echo $output;

function count_rat($business_id, $connect)
{
 $output = 0;
